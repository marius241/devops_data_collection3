...
# Définir le titre de la pages
st.markdown('<h1 class="titre_page">APPARTEMENTS À VENDRE</h1>', unsafe_allow_html=True)
st.markdown(
        """
        <div class="text">
    ...
    def extraire_page(selected_page):
        ...
        for p in range(1,selected_page+1):
            ...
            for conteneur in conteneurs:
                try:
                    link = 'https://dakarvente.com/annonces-categorie-appartements-vendre-61.html' + conteneur.find('a', class_="")['href']
...
if choix == "SCRAPER BRUT":
    # Chargement des données dans le DataFrame
    dataframe = pd.read_csv('data/appartements_vendre.csv')

    load_data(dataframe, 'Afficher les données', '1')
if choix == "SCRAPER CLEANING":
    ...
    def visualize_data(dataframe):
        ...
        if st.button('Graphique Des Prix'):
            ...
            data = data.sort_values('Prix(FCFA)', ascending=False)  # Tri croissant par prix
